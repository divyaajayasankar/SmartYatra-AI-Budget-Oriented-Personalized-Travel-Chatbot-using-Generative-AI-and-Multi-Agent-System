#!/usr/bin/env python3
"""
Setup script to create the models directory structure and files.
"""
...
def setup_models_directory():
    """Create the models directory and all required files."""
    
    # Define paths
    models_dir = r"d:\travel_chatbot_project\backend\models"
    requests_file = os.path.join(models_dir, "requests.py")
    ...
    # Create requests.py
    requests_content = '''"""
Pydantic models for travel chatbot API requests and responses.
"""
...
'''
    ...
    # Write requests.py
    print(f"Creating file: {requests_file}")
    try:
        with open(requests_file, 'w') as f:
            f.write(requests_content)
        print(f"✓ {requests_file} created successfully")
    except Exception as e:
    ...
    try:
        ...
        print(f"✓ {init_file} created successfully")
    except Exception as e:
    ...
    if os.path.isfile(requests_file):
        print(f"✓ {requests_file} exists")
    else:
        print(f"✗ {requests_file} does not exist")
        return False
